<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:72:"/www/wwwroot/a.topv.club/application/index/view/cdkey/cdkey_details.html";i:1572887350;s:59:"/www/wwwroot/a.topv.club/application/index/view/layout.html";i:1588953790;}*/ ?>
<!DOCTYPE html>
<html>
  <head>
...
-->
  ...
  <div class="element-box">
    <h5 class="form-header">
     <?php echo $cdkey_type_data['name']; ?>激活码 #<?php echo $cdkey_type_data['id']; ?>#<?php echo $cdkey_type_data['time']; ?>
    </h5>

    <div class="form-desc"  id="timer" >请于 <code>5分钟</code> 内操作完毕 <code>5分钟后</code> 本次会话将超时 从打开页面开始计算:
    </div>
    <!--------------------
    ...
    <div class="controls-above-table">
      <div class="row">
        <div class="col-sm-6">
          <span class="btn btn-sm btn-secondary">数量：<?php echo $cdkey_type_data['number']; ?></span>
          <span class="btn btn-sm btn-danger">已用：<?php echo $cdkey_type_data['invalid']; ?></span>
          <span class="btn btn-sm btn-secondary"><?php echo $cdkey_type_data['name']; ?></span>
        </div>
    ...
    <div class="table-responsive">
      ...
      <table class="table table-lightborder">
        ...
        <tbody>
          <?php if(is_array($cdkey_list) || $cdkey_list instanceof \think\Collection || $cdkey_list instanceof \think\Paginator): $i = 0; $__LIST__ = $cdkey_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo_cdkey_list): $mod = ($i % 2 );++$i;?> 
          <tr>
            <td>
              <?php echo $vo_cdkey_list['id']; ?>
            </td>
            <td>
              <?php echo $vo_cdkey_list['cdkey']; ?>
            </td>
            <td>
              <?php if($vo_cdkey_list['y'] == 1){ echo  date('Y-m-d H:i:s',$vo_cdkey_list['stime']); }else{ echo ''; } ?>
            </td>
            <td class="text-center">
             <?php echo $vo_cdkey_list['yid']; ?>
            </td>
            <td class="text-right">
              <div class="status-pill <?php if($vo_cdkey_list['y'] == 1){ echo 'red'; }else{ echo 'green'; } ?>" data-title="<?php if($vo_cdkey_list['y'] == 1){ echo '已使用'; }else{ echo '未使用'; } ?>" data-toggle="tooltip" data-original-title="" title=""></div>
            ...
            function CountDown() {
               ...
               //   if (maxtime == 2) document.all["timer"].innerHTML = '';
                    ...
                    clearInterval(timer);
                     ...
                     document.all["timer"].innerHTML = '出于安全原因,本次会话已超时！ 如需继续操作，请返回激活码列表再次访问！';
